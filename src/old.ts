...
const SKIP_VALUES = ["none", "initial", "transparent"];

/** our dark color scheme */
const COLORS = {
...
export const generateDarkModeStyles = (_: any, _2: any, doc: Document) => {
  ...
  doc.body.querySelectorAll(INLINE_STYLE_SELECTOR).forEach((e) => {
    ...
    for (const key of COLOR_STYLES as any[]) {
      ...
      if (style[key] && style[key] !== "initial") {
        element.setAttribute(`data-${key}-before`, style[key]);
        style[key] = generateInvertedColor(style[key], bgColor?.[1]);
      }
    }
    ...
    for (const attr of COLOR_ATTRS) {
      ...
      if (value) {
        element.setAttribute(`data-${attr}-before`, value);
        const inverted = generateInvertedColor(value, bgColor?.[1]);
